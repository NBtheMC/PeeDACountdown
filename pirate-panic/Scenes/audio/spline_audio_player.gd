@tool
class_name SplineAudioPlayer extends Path3D


@export var player_listeners : Array[NodePath]
...
func _start():
	...
	#check points
	get_child(0).progress_ratio = 1
	while get_child(0).progress > 0.5:
		get_child(0).progress -= 0.5
		check_points[0].append(get_child(0).get_child(0).global_transform.origin)
		check_points[1].append(get_child(0).progress)
	pass
...
func update(delta: float):
	for c in child_sounds:
		for sound in sounddict[c.name].size():
			if sound > 0:
				...
				sounddict[c.name][sound].volume_db = c.volume_db
	...
	if dist2 > 15:
		for follow in 6:
			get_child(follow).progress = check_points[1][closestInd] + sound_range*(follow-2.5)
	else:
		for follow in 3:
			get_child(follow).progress = check_points[1][closestInd] + sound_range*(follow-1.5)
		for follow in 3:
			get_child(follow+3).progress = check_points[1][closest2Ind] + sound_range*(follow-1.5)
	pass
